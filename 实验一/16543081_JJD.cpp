...
int compress1()
{
	char ifname[256] = "Pic.bmp"; 
	char ofname[256] = "Pic.bmp.huf";
	unsigned int i, j;
...
int extract123()
{
	char ifname[256] = "Pic.bmp.huf";
	char ofname[256] = "Pic2.bmp";
	unsigned int i;
...
int main()
{
	while (1)
	{
		...
		case 1: printf("Compressing……\n");
			flag = compress1();	// 压缩，返回值用于判断是否文件名不存在
			break;
		case 2: printf("Extracting……\n");
			flag = extract123();		// 解压，返回值用于判断是否文件名不存在
			break;
		}
		if (flag == -1)
